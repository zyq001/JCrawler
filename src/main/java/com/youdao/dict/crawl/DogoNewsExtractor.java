 ...
 */
@CommonsLog
public class DogoNewsExtractor extends BaseExtractor {

    public DogoNewsExtractor(Page page) {
        super(page);
    }

    public DogoNewsExtractor(String url) {
        super(url);
    }
    ...
    public boolean init() {
        log.debug("*****init*****");
        try {
            if(url.contains("video-of-the")){
                log.info("video of the week,skipped" + url);
                return false;
            }
            SoupLang soupLang = new SoupLang(SoupLang.class.getClassLoader().getResourceAsStream("DogoNewsRule.xml"));
...
//2015-09-12 08:25
        ...
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss X", Locale.US);
        Date date = null;
        try {
