 ...
 */
@CommonsLog
public class VOANewsExtractor extends BaseExtractor {

    public VOANewsExtractor(Page page) {
        super(page);
    }

    public VOANewsExtractor(String url) {
        super(url);
    }
    ...
    public boolean init() {
        log.debug("*****init*****");
        try {
            SoupLang soupLang = new SoupLang(SoupLang.class.getClassLoader().getResourceAsStream("VOANewsRule.xml"));
...
//        label = label.contains("China")?"China":label.contains("news")? "World": label;//news belong to World
    ...
    public boolean extractorTime() {
        ...
        SimpleDateFormat format = new SimpleDateFormat(M.toString() + " dd, yyyy hh:mm 'UTC'");
        Date date;
        try {
