 ...
 */
@CommonsLog
public class SCMPExtractor extends BaseExtractor {

    public SCMPExtractor(Page page) {
        super(page);
    }

    public SCMPExtractor(String url) {
        super(url);
    }
    ...
    public boolean init() {
        log.debug("*****init*****");
        try {
            SoupLang soupLang = new SoupLang(SoupLang.class.getClassLoader().getResourceAsStream("SCMPRule.xml"));
...
//        else
    ...
    public boolean extractorType() {
        ...
        if (elementType == null)
            return false;
        String type = elementType.attr("content");//select("h2").text();
//        child(0).tagName();
       ...
       /* if (host.equals(port)) return true;*/
        ...
        for (Element img : imgs) {
            try {
                ...
                img.removeAttr("typeof");
                img.removeAttr("width");
                img.removeAttr("class");
                img.removeAttr("height");
                img.removeAttr("title");
                img.removeAttr("alt");
                img.removeAttr("data-ignore");
...
//        if(nextPage.equals("")) return;
        SCMPExtractor extractor = new SCMPExtractor(url + "?singlepage=true");
        extractor.init();
