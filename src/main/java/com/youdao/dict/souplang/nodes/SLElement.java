 ...
 */
package com.youdao.dict.souplang.nodes;


import com.youdao.dict.souplang.Context;
import com.youdao.dict.souplang.LangNode;
    ...
    public Object process(Object input, Context context) throws InputTypeErrorException {
        ...
        if (cssSelector != null && !cssSelector.isEmpty()) {
            ...
            if (jsoupElement != null) {
                Elements elements = jsoupElement.select(cssSelector);
                if ( elements!= null && elements.size() > 0)
                    result = jsoupElement.select(cssSelector).get(index);
            } else {
                Elements elements = jsoupElements.select(cssSelector);
                if ( elements != null && elements.size() > 0)
