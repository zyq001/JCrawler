...
public class TutorialCrawler extends DeepCrawler {
    ...
    public Links visitAndGetNextLinks(Page page) {
        try {
            ParserPage p = ChinaDailyExtractor.extractor(page);
            if (p != null && jdbcTemplate != null) {
                int updates = jdbcTemplate.update("insert into parser_page2 (title, type, label, level, style, host, url, content, version, mainimage) values (?,?,?,?,?,?,?,?,?,?)",
