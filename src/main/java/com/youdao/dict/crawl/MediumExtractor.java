 ...
 */
@CommonsLog
public class MediumExtractor extends BaseExtractor {

    public MediumExtractor(Page page) {
        super(page);
    }

    public MediumExtractor(String url) {
        super(url);
    }
    ...
    public boolean init() {
        log.debug("*****init*****");
        try {
            SoupLang soupLang = new SoupLang(SoupLang.class.getClassLoader().getResourceAsStream("MediumRule.xml"));
...
//                content.select(".progressiveMedia-thumbnail").remove();
//                content.select(".js-progressiveMedia-inner").remove();
...
//            System.out.println(a);
        ...
        content.select(".graf--h3").remove();//去除重复的题目-Medium
...
//        else
    ...
    public boolean extractorType() {
        Elements elementType = doc.select(".tags");
        if (elementType == null){
...
//        p.setType(type.trim());
        ...
        String[] keywords = new String[tagSize];
        for(int i = 0; i < tagSize; i++){
            String keyword = tags.get(i).text();
            keywords[i] = keyword;

            //遍历关键字中第一个能确定频道的
            if(newType == null &&(TypeDictHelper.rightTheType(keyword)))//正好是频道之一，
                newType = keyword;
            else if(newType == null && !keyword.equals(TypeDictHelper.getType(keyword, keyword)))//不是频道，但是能在字典中查到
                newType = TypeDictHelper.getType(keyword, keyword);
        }
...
//        if(isPaging()) return true;
       /* if (host.equals(port)) return true;*/
        ...
        content.select(".progressiveMedia-thumbnail").remove();//缩略图
        content.select(".js-progressiveMedia-inner").remove();//重复的原图
